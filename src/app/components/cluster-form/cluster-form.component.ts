...
export class ClusterFormComponent implements OnInit {
  ...
  removeAddress(address) {
    this.cluster.addresses = this.cluster.addresses.filter(
      (existingAddress) => {
  ...
  save() {
    const savedCluster =
      this.cluster.name && this.cluster.description ? this.cluster : null;
  ...
  delete() {
    if (this.numPolicies > 0) {
      ...
      this.toastr.error(
        `You cannot delete this cluster as it has ${this.numPolicies} configured ${pluralizedTerm}`
      );
    } else {
